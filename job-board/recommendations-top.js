...
(() => {
  try {
    ...
    if (document.querySelector('#aiRecommendationsTop')) return;
    ...
    const recommendedCount = document.querySelector('#recommendedTabCount');
    if (recommendedCount) {
      new MutationObserver(sync).observe(recommendedCount, { childList: true, characterData: true, subtree: true });
    }
    sync();
  } catch (error) {
    console.error('Recommendations top enhancement skipped:', error);
  }
})();
