...
scene.fog = new THREE.Fog(0xf0f4f8,48,95)
...
scene.add(new THREE.HemisphereLight(0xffffff,0x8b9aad,2.6))
...
function label(text,color,small=true){
  const c=document.createElement('canvas'); c.width=720; c.height=small?100:210
  const x=c.getContext('2d'); x.clearRect(0,0,c.width,c.height)
  if(small){
    x.fillStyle='rgba(255,255,255,.82)'; x.roundRect(70,22,c.width-140,c.height-44,18); x.fill()
    x.strokeStyle='rgba(19,31,48,.16)'; x.lineWidth=2; x.stroke()
    x.textAlign='center'; x.textBaseline='middle'; x.fillStyle='#152235'; x.font='700 27px Arial';
    x.shadowColor='rgba(255,255,255,.9)'; x.shadowBlur=2; x.fillText(text,c.width/2,c.height/2)
  }else{
    x.fillStyle='rgba(8,13,22,.90)'; x.roundRect(16,16,c.width-32,c.height-32,26); x.fill()
    x.strokeStyle=color; x.lineWidth=5; x.stroke()
    x.textAlign='center'; x.textBaseline='middle'; x.fillStyle='#fff'; x.font='900 62px Arial'; x.fillText(text,c.width/2,c.height/2)
  }
  const t=new THREE.CanvasTexture(c); t.colorSpace=THREE.SRGBColorSpace
  const s=new THREE.Sprite(new THREE.SpriteMaterial({map:t,transparent:true,depthTest:false}))
  s.scale.set(small?.78:3.2,small?.11:.82,1)
  return s
}
...
function desk(category,name){
  const g=new THREE.Group()
  const top=box(1.35,.08,.72,0xf7f8fa,{roughness:.84,metalness:.03}); top.position.y=.76; g.add(top)
  for(const [x,z] of [[-.54,-.26],[-.54,.26],[.54,-.26],[.54,.26]]){const l=box(.06,.7,.06,0x8b97a7,{metalness:.22}); l.position.set(x,.35,z); g.add(l)}
  const mon=box(.54,.34,.04,0x182333,{roughness:.16,metalness:.18}); mon.position.set(0,1.03,-.20); mon.material.emissive=new THREE.Color(category.color); mon.material.emissiveIntensity=.35; g.add(mon)
  const chair=box(.36,.42,.36,0x536983,{roughness:.78}); chair.position.set(0,.48,.50); g.add(chair)
  const p=person(category.color); p.position.set(0,0,.35); g.add(p)
  const tag=label(name,category.color,true); tag.position.set(0,1.32,.38); g.add(tag)
  return g
}

const floor=box(34,.18,30,0xf7f8fb,{roughness:.96,castShadow:false}); floor.position.y=-.09; scene.add(floor)
const aisle=box(4.2,.02,29,0xcfd8e4,{roughness:1,castShadow:false}); aisle.position.y=.011; scene.add(aisle)
const back=box(34,6,.22,0xe7edf4,{roughness:.94}); back.position.set(0,3,-15); scene.add(back)
for(const x of [-17,17]){const glass=box(.16,5.8,29,0xcfe3f7,{transparent:true,opacity:.28,roughness:.08,metalness:.12}); glass.position.set(x,2.9,0); scene.add(glass)}
for(let x=-12;x<=12;x+=6){for(let z=-11;z<=11;z+=7){const panel=box(3.4,.05,1.1,0xffffff,{roughness:.12,castShadow:false}); panel.position.set(x,5.7,z); scene.add(panel); const l=new THREE.PointLight(0xffffff,1.05,10,2); l.position.set(x,5.2,z); scene.add(l)}}

const loungeFloor=box(5.4,.03,4.2,0xe6ebf1,{roughness:.98}); loungeFloor.position.set(0,.015,0); scene.add(loungeFloor)
const loungeTable=box(1.9,.1,.9,0xc6d0dc,{metalness:.12}); loungeTable.position.set(0,.72,0); scene.add(loungeTable)
for(const [x,z,r] of [[-1.7,0,Math.PI/2],[1.7,0,Math.PI/2],[0,-1.3,0],[0,1.3,0]]){const s=box(1.3,.58,.72,0x7088a6,{roughness:.84}); s.position.set(x,.3,z); s.rotation.y=r; scene.add(s)}
...
function addDepartment(category){
  ...
  const zone=box(width,.025,depth,category.accent,{transparent:true,opacity:.42,roughness:.98,castShadow:false}); zone.position.y=.015; g.add(zone)
  ...
  const light=new THREE.PointLight(category.color,.85,9,2); light.position.set(0,3,0); g.add(light)
...
for(const [x,z] of [[-14,-12],[14,-12],[-14,11],[14,11],[-2.7,4],[2.7,4]]){const pot=box(.42,.36,.42,0x8795a6); pot.position.set(x,.18,z); scene.add(pot); const trunk=box(.08,.8,.08,0x806447); trunk.position.set(x,.68,z); scene.add(trunk); const leaves=new THREE.Mesh(new THREE.SphereGeometry(.46,16,14),new THREE.MeshStandardMaterial({color:0x5aa87a,roughness:.95})); leaves.position.set(x,1.24,z); scene.add(leaves)}
...
function focusDepartment(id){
  ...
  sections.forEach(({light,zone},key)=>{light.intensity=key===id?1.45:.65; zone.material.opacity=key===id?.58:.34})
  const pos=new THREE.Vector3(); s.group.getWorldPosition(pos)
  const offset=s.layout.x<0?new THREE.Vector3(-6.5,6.5,2.5):s.layout.x>0?new THREE.Vector3(6.5,6.5,2.5):new THREE.Vector3(0,6.8,8)
  targetCamera={position:pos.clone().add(offset),target:pos.clone().add(new THREE.Vector3(0,1,0))}
}

function resetView(){
  targetCamera={position:new THREE.Vector3(0,25,37),target:new THREE.Vector3(0,1,0)}
  sections.forEach(({light,zone})=>{light.intensity=.85; zone.material.opacity=.42})
}

populateUI(); resetView()
const resetButton=document.querySelector('#reset-view'); if(resetButton) resetButton.addEventListener('click',resetView)
