...
scene.fog = new THREE.Fog(0x07101a, 42, 88)
...
function textSprite(text, color = '#ffffff', small = false) {
  ...
  x.fillStyle = small ? 'rgba(4,8,13,.74)' : 'rgba(4,8,13,.9)'
  x.roundRect(12, 12, 616, c.height - 24, small ? 20 : 28)
  x.fill()
  x.strokeStyle = color
  ...
  x.fillStyle = '#fff'
  x.font = small ? '700 36px Arial' : '800 56px Arial'
  x.fillText(text, 320, c.height / 2)
  const texture = new THREE.CanvasTexture(c)
  texture.colorSpace = THREE.SRGBColorSpace
  const material = new THREE.SpriteMaterial({ map: texture, transparent: true, depthTest: false })
  const sprite = new THREE.Sprite(material)
  sprite.scale.set(small ? 1.55 : 2.55, small ? 0.31 : 0.72, 1)
  return sprite
}

const floor = box(48, 0.35, 38, 0x111a27, { roughness: 0.75, metalness: 0.08 })
floor.position.y = -0.18
scene.add(floor)
const carpet = box(45.8, 0.05, 35.8, 0x172334, { roughness: 0.92, metalness: 0 })
...
for (const [w, h, d, x, y, z] of [[48,5.4,.08,0,2.7,-19],[48,5.4,.08,0,2.7,19],[.08,5.4,38,-24,2.7,0],[.08,5.4,38,24,2.7,0]]) {
...
for (let x = -20; x <= 20; x += 5) {
  const beam = box(0.12, 0.12, 36, 0x24344b, { metalness: 0.5, roughness: 0.35 })
  beam.position.set(x, 5.35, 0)
  scene.add(beam)
  for (let z = -15; z <= 15; z += 5) {
    const lamp = new THREE.PointLight(0xd7e7ff, 1.2, 8.5, 2)
...
function createDesk(color) {
  ...
  const top = box(1.35, 0.10, 0.70, 0x7f6854, { roughness: 0.68, metalness: 0.04 })
  top.position.y = 0.76
  desk.add(top)
  ...
  const screen = box(0.58, 0.38, 0.035, color, { roughness: 0.18, metalness: 0.2 })
...
const clickable = []
const zoneLayout = [[-15,-10],[-5,-10],[5,-10],[15,-10],[-13,7],[0,8],[13,7]]
...
function addCategory(category, categoryIndex) {
  ...
  const colSpacing = count >= 10 ? 1.85 : 2.0
  const rowSpacing = 2.35
  const zoneW = Math.max(5.2, columns * colSpacing + 1.4)
  const zoneD = Math.max(4.0, rows * rowSpacing + 1.5)
  ...
  title.position.set(0, 3.15, -zoneD / 2 + 0.15)
  group.add(title)
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < columns; col++) {
      ...
      const z = (row - (rows - 1) / 2) * rowSpacing + 0.45
      ...
      nameTag.position.set(0, 2.22, 0.60)
      station.add(nameTag)
  ...
  const light = new THREE.PointLight(color, 4.6, 9, 2)
...
const rug = box(8.5, 0.035, 5.5, 0x253247, { roughness: 0.95, metalness: 0 })
rug.position.y = 0.07
lounge.add(rug)
const table = new THREE.Mesh(new THREE.CylinderGeometry(1.25, 1.25, 0.12, 48), new THREE.MeshStandardMaterial({ color: 0x9a7a58, roughness: 0.55 }))
table.position.y = 0.68
lounge.add(table)
for (let a = 0; a < 8; a++) {
  const chair = box(0.55, 0.55, 0.55, 0x35445a, { roughness: 0.7 })
  const angle = (a / 8) * Math.PI * 2
  chair.position.set(Math.cos(angle) * 2.0, 0.3, Math.sin(angle) * 2.0)
  lounge.add(chair)
}
scene.add(lounge)

for (const [x,z] of [[-22,-16],[22,-16],[-22,16],[22,16],[-22,0],[22,0],[-4,16],[4,16]]) {
...
function selectCategory(id, focusCamera = true) {
  ...
  document.querySelector('#detail-note').innerHTML = `<strong>Vizibilitate:</strong> birourile sunt mai aerisite, iar numele sunt ridicate și mai ușor de citit din vedere generală.`
  ...
  categoryObjects.forEach(({ group, light }, key) => {
    ...
    light.intensity = active ? 7 : 1.8
  })
  if (focusCamera) {
    const pos = new THREE.Vector3(); obj.group.getWorldPosition(pos)
    targetCamera = { pos: pos.clone().add(new THREE.Vector3(10.5,9.5,11.5)), target: pos.clone().add(new THREE.Vector3(0,1.0,0)) }
  }
}

function resetView() {
  targetCamera = { pos: new THREE.Vector3(32,28,38), target: new THREE.Vector3(0,1.1,0) }
